...
class HARDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = np.asarray(
            self.labels[index * self.window_size : (index + 1) * self.window_size]
        )[-1]
...
def load_dataset(args):
    datasets = {x: HARDataset(args=args, phase=x) for x in ["train", "val"]}
    data_loaders = {
        x: DataLoader(
            datasets[x],
...
def load_classifier_dataset(args):
    ...
    data_loaders = {
        x: DataLoader(
            datasets[x],
            batch_size=args.classifier_batch_size if x == "train" else 256,
            shuffle=True if x == "train" else False,
            num_workers=2,
            pin_memory=True,
        )
