...
def model_save_name(args, classifier=False, capture=False):
    if capture:
        cpc = (
            "capture_24_k_{0.num_steps_prediction}_neg_"
            "{0.num_negatives}_enc_{0.input_downsampling}"
            "_agg_{0.aggregator_type}".format(args)
        )
    else:
        cpc = (
            "{0.dataset}_k_{0.num_steps_prediction}_neg_"
            "{0.num_negatives}_enc_{0.input_downsampling}"
            "_agg_{0.aggregator_type}".format(args)
        )
    ...
    training_settings = (
        "_lr_{0.learning_rate}_wd_{0.weight_decay}_bs" "_{0.batch_size}".format(args)
    )
    ...
    if classifier:
        ...
        classification += (
            ...
            "{0.classification_model}".format(args)
        )

    # Different random seed
    random_seed = "_rs_{0.random_seed}".format(args) if not capture else "_rs_42"
    ...
    print("The save name is: {}".format(name))
...
def update_args(config, args):
    ...
    if "encoder_only" in config:
        args.encoder_only = config["encoder_only"]
